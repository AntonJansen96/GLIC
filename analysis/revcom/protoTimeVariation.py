...
blocks = [[13, 14, 26, 31, 32, 35, 49, 55], [67, 69, 75, 82, 86, 88, 91, 97], [104, 115, 122, 127, 136, 145, 147, 153], [154, 161, 163, 177, 178, 181, 185, 222], [235, 13, 13, 13, 243, 272, 277, 282]]
...
for block in blocks:
    ...
    for simidx in range(0, nsims):
        ...
        for residx in range(0, len(block)):
            ...
            for rep in reps:
                for chain in chains:
                    x = [1 - val for val in superData[block[residx]][sims[simidx]][rep][chain]]
                    t = range(len(x))
            ...
            t = range(len(x))
            subplt.plot(t, x, linewidth=2, color='black')
    ...
    fig.savefig(f"variation_{block[0]}.png")
    os.system(f"convert variation_{block[0]}.png -trim variation_{block[0]}.png")
    fig.clear()
