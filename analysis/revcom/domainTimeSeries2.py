...
for row in range(0, nrows):
    for col in range(0, ncols):
        ...
        for chain in chains:
            x = superData[sims[col]][reps[row]][chain]["beta_expansion"]
            t = range(0, len(x))
        ...
        # Set x-lim and y-lim.
        subplt.set_ylim([11, 22]) # ECD Twist.
        subplt.set_xlim([0, 1000])

        subplt.text(15, 20.5, str(row+1))
        ...
        if col != 0:
            ...
            subplt.set_ylabel(r'$\mathrm{\beta}$-expansion ($\AA$)', size=20)
            subplt.set_yticks([12, 14, 16, 18, 20])
...
fig.savefig('betaexpansion.png')
fig.clf()
os.system("convert betaexpansion.png -trim betaexpansion.png")
