...
class PanelBuilder:
    ...
    def __init__(self, target, resids, rmsd='', test=False):
        ...
        def __task(sim, rep):
            ...
            fname = "panels/proto_{}_{}_{}.png".format(sim, rep, self.target)
            if not exists(fname) and self.target not in [127, 235, 277]:
                self.chargePlot(sim, rep)
            else:
                print("{} already exists, not creating it again...".format(fname))

            # CREATE THE MINIMUM DISTANCE PLOTS
            for resid in self.resids:
                fname = "panels/mindist_{}_{}_{}-{}.png".format(sim, rep, self.target, resid)
                if not exists(fname):
            ...
            if self.rmsd != '':
                fname = "panels/rmsd_{}_{}_{}.png".format(sim, rep, self.target)
                if not exists(fname):
    ...
    def chargePlot(self, sim, rep):
        ...
        chain = ['A', 'B', 'C', 'D', 'E']
        array = getLambdaFileIndices('../sims/{}/{:02d}/CA.pdb'.format(sim, rep), self.target)

        store = []
        for idx in range(0, len(array)):
            data = loadxvg('../sims/{}/{:02d}/cphmd-coord-{}.xvg'.format(sim, rep, array[idx]), dt=5000, b=0)
    ...
    def mindistPlot(self, sim, rep, resid):
        ...
        os.chdir('../sims/{}/{:02d}'.format(sim, rep))
        ...
        for idx in range(0, len(chain1)):
            ...
            with open('../../../analysis/panels/occ_{}_{}_{}-{}.txt'.format(sim, rep, self.target, resid), 'a+') as file:
    ...
    def addResidueLetters(self, residList, fileName='../sims/4HFI_4/01/CA.pdb'):
