...
source = pickle.load(open('/home/anton/GIT/GLIC/analysis/revcom/protoTimeCompact.obj', 'rb'))
df = pd.DataFrame()
    ...
    for sim in sims:
        for rep in reps:
            for chain in chains:
                ...
                array = source[resid][sim][rep][chain]

                while len(array) < 1001:  # Some sims finished a few ns before 1000.
                    array += [array][-1]

                df[f'{resid}_{sim}_{rep}_{chain}'] = source[resid][sim][rep][chain][0:1001]

df.to_csv('movingproto.csv')

#! THIS PART LOADS THE .CSV FILE ###############################################

superData = makeSuperDict([resids, sims, reps, chains, []])  

df = pd.read_csv('movingproto.csv')
for resid in resids:
    for sim in sims:
        for rep in reps:
            for chain in chains:
                superData[resid][sim][rep][chain] = list(df[f'{resid}_{sim}_{rep}_{chain}'])

#! THIS PART PERFORMS FURTHER DATA PROCESSING ##################################
